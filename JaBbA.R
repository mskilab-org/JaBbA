...
ISVAHOME = paste(Sys.getenv('GIT_HOME'), '/isva/', sep = "/");
GATK.JAR = '/humgen/gsa-hpprojects/GATK/bin/current//GenomeAnalysisTK.jar';
GATK.REFDIR = '/seq/references/Homo_sapiens_assembly19/v1/';
GATK.HG19 = paste(GATK.REFDIR, 'Homo_sapiens_assembly19.fasta', sep = "") 
GATK.DBSNP = paste(GATK.REFDIR, 'Homo_sapiens_assembly19.dbsnp.vcf', sep = "");
SAMTOOLS.PATH = '/broad/software/free/Linux/redhat_5_x86_64/pkgs/samtools/samtools_0.1.16/bin/samtools';
SAMTOOLS.FA = GATK.HG19
GC.WIG.DIR = '~/DB/Tracks/gccontent'
MAP.WIG.DIR = '~/DB/Tracks/mappability'
Sys.setenv(ILOG_LICENSE_FILE = '')
##
##
...
##
## to instal igraph, need a "use GCC-4.8"
##
##
source(paste(Sys.getenv('GIT_HOME'), 'grUtils/grUtils.R', sep = "/"))
source(paste(Sys.getenv('GIT_HOME'), 'ffTrack/ffTrack.R', sep = "/"))

##########
# karyograph
#
# builds graph from rearrangement breakpoints +/- copy number endpoints
# used for downstream raMIP and karyoMIP functions
#
...
#
# The output is a list with the following fields
...
#      important:  $type specifies which edges are "aberrant" and "reference", $bp.id specifies which input rearrangement (item in bpp)
...
# $ab.edges = length(bpp) x {'from', 'to'} x {'+', '-'} mapping junction id's (indices into input bpp lists) to source and sink vertices,
#             in both orientations 
#
# 
##########
karyograph = function(bpp, ## this is a grl of breakpoint pairs (eg output of ra_breaks(dranger.df) where dranger is df of dranger output)
  ...
  )
  {
    ...
    if (length(bpp)>0)
      {
        ...
        ### HACK HACK to force seqlengths to play with each other if malformed
        ...
        if (is.null(tile))
            {
                ...
                return(
                    list(tile = tile, adj = A,
                         G = graph.adjacency(A), ab.adj = A != 0, ab.edges = NULL, junctions = bpp))
            }
    ...
    if (!is.null(tile))
      {
        ...
        if (!identical(sort(seqlevels(tile)), seqlevels(bpp)))
          {
            tile = gr.fix(tile, bpp)
            bpp = gr.fix(bpp, tile)
          }

        if (!identical(sort(seqlevels(tile)), seqlevels(bp1)))
          {
            tile = gr.fix(tile, bp1)
            bp1 = gr.fix(bp1, tile)
            bp2 = gr.fix(bp2, tile)
    ...
    if (length(bpp)>0)
      g = gaps(gr.stripstrand(sort(grbind(bp1, bp2, tbp))))
    else
    ...
    if (length(junc.bpix)>0)
      {        
        ...
        #
          ...
          ab.pairs = cbind(
              ifelse(as.logical(strand(bp1)=='+'), gr.match(shift(gr.start(bp1), 1), gr.start(tile)), 
                     gr.match(gr.start(bp1), gr.end(tile))),
              ifelse(as.logical(strand(bp2)=='+'), gr.match(shift(gr.start(bp2), 1), gr.start(tile)),
...
#    ref.pairs = ref.pairs[ref.pairs[,1]>0 & ref.pairs[,2]!=length(tile), ]
    ...
    ab.edges = array(NA, dim = c(length(bpp), 3, 2), dimnames = list(NULL, c('from', 'to', 'edge.ix'), c('+', '-')))
    ...
    return(list(tile = tile, adj = adj, G = G, ab.adj = adj.ab != 0, ab.edges = ab.edges, junctions = bpp))
  }
  ...
  )
{
  ...
  #####
  # wrapper that calls raMIP recursively on subgraphs after "fixing"
  #####
...
#        ss[ss$loose] = gr.flip(ss[ss$loose]) + 100
    ...
    out$td = trackData(ss, y.field = 'cn', edges = out$edges[order(out$edges$cn), ], name ='JaBbA', angle = 0)
...
####################################################################
# raMIP.process
# process raMIP solution "sol" given original graph "g" (karyograph() list output)
#
# output is list with items
...
#
###################################################################
raMIP.process = function(
  ## output of raMIP, sol$segstats needs to have field $tile.id whose unique values appear exactly twice in the object, 
  ...
  )
  {
    ...
    if (!all(c('segstats', 'adj') %in% names(sol)))
      stop('sol must be output of raMIP()')
...
# raMIP.allelic
#
# Takes adj and segstats from output from raMIP and
# a granges of het.sites with $ref.count and $alt.count
# read count and outputs list with fields
...
####################
# 
# takes "total copy" reconstruction and populates allelic value s
#
#
####################    
...
# and numeric vector specifying node copy numbers.  Returns a list
# with fields
...
# loose.ends
#
# takes raMIP output and outputs a vector of ranges
...
# slack.breaks
#
# makes "breaks" GRL from slack edges in raMIP segstats output (GRanges object)
#
...
slack.breaks = function(segstats)
  {
    if (any(!(c('eslack.out', 'eslack.in') %in% names(values(segstats)))))
      stop('Input segstats should be $segstats field output of raMIP, i.e. be a GRAnges with fields eslack.out and eslack.in')
...
# and length |E| vector of edge copy numbers (eclass), length |E| vector of edge equivalence classes (both outputs of raMIP.process)
...
#  B, ## B matrix from which K was generated (vertices x edges), the rownames of B should correspond to indices of gr (if gr is supplied)
  ...
  )
{
  ...
  contigs = contigs[!duplicated(sol$kclass[contigs])]
...
##       sG = graph(rbind(so, si))
      ...
      if (out$is.cyc[i])
        {
          p.fwd = get.shortest.paths(sG, v.all[1], v.all[pmin(length(v.all), 2)])
          p.bwd = get.shortest.paths(sG, v.all[pmin(length(v.all), 2)], v.all[1])
      ...
      else
        {
          ...
          return(unlist(get.shortest.paths(sG, v.in, v.out)))
        }
    }, mc.cores = mc.cores)
...
# jabba.hood
#
# Given "jabba" structure (list with $segstats and $adj matrix)
...
# jabba.dist
#
# Given "jabba" structure and input granges gr1 and gr2 of (signed) intervals
# returns a length(gr1) x length(gr2) matrix whose entries ij store the distance between
...
jabba.dist = function(jab, gr1, gr2,
  ...
  max.dist = Inf, ## if max.dist is not Inf then a sparse matrix will be returned that has 0 at all
...
# jabba.walk
#
# Takes in karyograph and JaBbA solution and computes local
...
jabba.walk = function(sol, kag = NULL, digested = T, outdir = 'temp.walk', junction.ix = NULL, loci = NULL, clustersize = 100,
  trim = T, ## whether to trim around junction (only applicable when loci = NULL)
  trim.w = 1e6, ## how far to trim in neighborhood of junction (only applicable when loci = NULL
  prune = F, ## whether to prune trivial walks ie those for whom a path can be drawn from first to last interval in a graph linking intervals with pairwise distance < d1 on the walk or distance < d2 on the reference
  prune.d1 = 1e5, ## local distance threshold for walk pruning 
  prune.d2 = 1e5, ## reference distance threshold for walk pruning
  maxiterations = Inf, mc.cores = 1, genes = read.delim('/cga/meyerson/home/marcin/DB/COSMIC/cancer_gene_census.tsv', strings = F)$Symbol, verbose = T, max.threads = 4, customparams = T, mem = 6, all.paths = FALSE, nomip = F, tilim = 100, nsolutions = 100, cb.interval = 1e4, cb.chunksize = 1e4, cb.maxchunks = 1e10)
{
  ...
  if (!is.null(genes))
    td.rg = track.refgene(genes = genes, height = 3)
  ...
  if (length(loci)>0)
    {
      out = mclapply(1:length(loci), function(i)
        {          
          ...
          karyo.sol = karyoMIP(K, h$e[eix], h$eclass[eix], nsolutions = nsolutions, tilim = tilim)
          kag.sol = karyo.sol[[1]]
#          p = karyoMIP.to.path(kag.sol, K, as.matrix(h$B)[vix, eix], sol$segstats, mc.cores = pmin(4, mc.cores))
#          p = karyoMIP.to.path(kag.sol, K, Bc, sol$segstats, mc.cores = pmin(4, mc.cores))
          ...
          pdf(outfile.pdf, height = 16, width = 24)
          ...
          if (!is.null(sol$td))
            ...
            td.seg = trackData(sol$segstats, y.field = 'cn', angle = 0, col ='black', height = 6, labels.suppress = T, y1 = y1)
          
          td = c(trackData(grs, draw.paths = T, path.cex.arrow = 0, border = NA, angle = 0, ywid = 0.5, path.stack.x.gap = 1e6, height = 20, labels.suppress.gr = T),
            td.seg,
            td.rg)
          display(td,
          ...
          if (all.paths)
            {
              ...
              allp = karyoMIP.to.path(list(kcn = kag.sol$kcn*0 + 1, kclass = kag.sol$kclass), K, h$e.ij[eix, ], sol$segstats, mc.cores = pmin(4, mc.cores), verbose = verbose)$grl
#              allp = karyoMIP.to.path(list(kcn = kag.sol$kcn*0 + 1, kclass = kag.sol$kclass), K, as.matrix(h$B)[vix, eix], sol$segstats, mc.cores = pmin(4, mc.cores), verbose = verbose)$grl
...
#                win = streduce(unlist(allps), sum(width(unlist(allps)))*0)
              ...
              values(allps) = NULL
              out$allpaths = allps             
              out$td.allpaths = c(trackData(out$allpaths, draw.paths = T, path.cex.arrow = 0, border = NA, angle = 0, ywid = 0.5, path.stack.x.gap = 1e6, height = 20, labels.suppress.gr = T),
                td.seg,
                td.rg)
              pdf(outfile.allpaths.pdf, height = 16, width = 24)
              display(out$td.allpaths,
                      windows = win, links = kag$junctions)
              dev.off()                                  
              
              out$README = paste(out$README, 'allpaths= all paths through windows (not just optimal ones), td.allpaths = trackData object of plot of all paths')
            }
...
# reads.to.walks
#
# Takes input bam and collection of walks (GRanges list of signed intervals on hg19 or other BSgenome)
# and uses bwa mem to realign reads to walks, returns paths to bams
#
#
##############################################
...
# 
# returns a grl of putative fusions with annotations in values field:
# $label
...
#
###############################################
annotate.walks = function(walks, cds, promoters = NULL, filter.splice = T, verbose = F, prom.window = 1e3, mc.cores = 1)
    {
        require(igraph)
        ...
        cds.span = seg2gr(values(cds))

        names(values(cds.span))[match(c('Transcript_id', 'Transcript_name', 'Gene_name'), names(values(cds.span)))] = c('transcript_id', 'transcript_name', 'gene_name')
        
        if (is.null(promoters))
            {
                promoters = flank(cds.span, prom.window)
                values(cds.span) = values(promoters)
            }
        
        cds.span$type = 'cds'
        promoters$type = 'gene'
        tx.span = seg2gr(grdt(grbind(cds.span, promoters))[, list(seqnames = seqnames[1], start = min(start), end = max(end), strand = strand[1], gene_name = gene_name[1], transcript_name = transcript_name[1]), keyby = transcript_id], seqlengths = seqlengths(cds.span))
                        
        # match up tx.span to walks
        walks.u = grl.unlist(walks)

        ## these are fragments of transcripts that overlap walks
        this.tx.span = gr.findoverlaps(tx.span, walks.u, qcol = c('transcript_id', 'transcript_name', 'gene_name'))
        ...
        this.tx.span$left.feat[gr.findoverlaps(gr.start(this.tx.span), cds.span, by = 'transcript_id')$query.id] = 'cds'
        this.tx.span$left.feat[gr.findoverlaps(gr.start(this.tx.span), promoters, by = 'transcript_id')$query.id] = 'promoter'
                   
        this.tx.span$right.feat = NA
        this.tx.span$right.feat[gr.findoverlaps(gr.end(this.tx.span), cds.span, by = 'transcript_id')$query.id] = 'cds'
        this.tx.span$right.feat[gr.findoverlaps(gr.end(this.tx.span), promoters, by = 'transcript_id')$query.id] = 'promoter'
        ...
        tix = match(this.tx.span$transcript_id, cds.span$transcript_id)
        cds.u = grl.unlist(cds[tix])
        ranges(cds.u) =  ranges(pintersect(cds.u, cds.span[tix[cds.u$grl.ix]], resolve.empty = 'start.x'))
        
        tmp = gr.findoverlaps(this.tx.span, cds.u, scol = c('start.local', 'end.local', 'exon_number'), by = 'transcript_id', verbose = verbose)

        leftmost.cds.exon = data.table(id = 1:length(tmp), qid = tmp$query.id)[, id[which.min(start(tmp)[id])], by = qid][, V1]
        rightmost.cds.exon = data.table(id = 1:length(tmp), qid = tmp$query.id)[, id[which.max(end(tmp)[id])], by = qid][, V1]
        ...
        # paying attention to (1) side of breakage, (2) orientation of adjacent windows and (3) orientation of transcript
        #
        # i.e. right broken + cds in + window will attach to a left broken + cds in the next + window
        #                                               or a right broken - cds in the next - window
        #      left broken - cds in + window will attach to a right broken - cds in the next + window
        #                                               or a left broken + cds in the next - window                                             
        #      left broken - cds in + window will attach to a right broken - cds in the next + window
        #                                               or a left broken + cds in the next - window                                             
        ...
        edges = merge(data.frame(i = 1:length(this.tx.span), key1 = walks.u$grl.ix[this.tx.span$subject.id], key2 = walks.u$grl.iix[this.tx.span$subject.id], key3 = this.tx.span$cds.sign*this.tx.span$window.sign,
            key4 = ifelse(this.tx.span$window.sign>0, this.tx.span$right.broken, this.tx.span$left.broken)),
            data.frame(j = 1:length(this.tx.span), key1 = walks.u$grl.ix[this.tx.span$subject.id],
                       key2 = walks.u$grl.iix[this.tx.span$subject.id]-1, key3 = this.tx.span$cds.sign*this.tx.span$window.sign,
        ...
        paths = do.call('c', mclapply(1:length(vL), function(i) {
            ...
            if (length(x)==2)
                ...
                lapply(all.paths(A[x,x, drop = FALSE], source.vertices = tmp.source, sink.vertices = tmp.sink, verbose = FALSE)$paths, function(y) x[y])
                                        ...
                                        #    paths = all.paths(A, source.vertices = sources, sink.vertices = sinks, verbose = verbose)$paths ## list of indices in this.tx.span corresponding to transcript walks
        ...
        paths.u.lout = tmp.gr$left.feat %in% c('promoter')
        paths.u.rout = tmp.gr$right.feat %in% c('promoter')
 ...
 #    values(fusions)$walk.id = vaggregate(walks.u$grl.ix[tmp.gr$subject.id], by = list(paths.i), FUN = function(x) x[1])
        ...
        values(fusions)[, 'coords'] = mapply(function(x) paste(unique(x), collapse = '; '),
                           split(gr.string(this.tx.span[paths.u], mb = TRUE, round = 1), paths.i))
        
        values(fusions)[, 'transcript_names'] = mapply(function(x, y) paste(x, ' (', y, ')', sep = '', collapse = '; '),
                           split(values(tx.span)[, 'gene_name'][this.tx.span$query.id[paths.u]], paths.i), 
                           split(values(tx.span)[, 'transcript_name'][this.tx.span$query.id[paths.u]], paths.i))

        values(fusions)[, 'transcript_ids'] = mapply(function(x, y) paste(x, ' (', y, ')', sep = '', collapse = '; '),
                           split(values(tx.span)[, 'gene_name'][this.tx.span$query.id[paths.u]], paths.i), 
                           split(values(tx.span)[, 'transcript_id'][this.tx.span$query.id[paths.u]], paths.i))

        values(fusions)[, 'genes'] = mapply(function(x) paste(unique(x), collapse = '; '),
                           split(values(tx.span)[, 'gene_name'][this.tx.span$query.id[paths.u]], paths.i))
...
#
# gene_name is used in annotation and in not creating "splice" fusions that arise from different transcripts of the same gene.
#
# 
# returns a GRangesList of walks corresponding to transcript boundaires
#####################################
fusions = function(junctions = NULL, jab = NULL, cds = NULL, promoters = NULL, prom.window = 1e3, verbose = T,
    cb.interval = 1e4, cb.chunksize = 1e4, cb.maxchunks = 1e10)
    {
        ...
        names(values(cds.span))[match(c('Transcript_id', 'Gene_name'), names(values(cds.span)))] = c('transcript_id', 'gene_name')
        ...
        cds.frag.left = gr.findoverlaps(cds.span, gr.start(seg), qcol = c('gene_name', 'transcript_id'), ignore.strand = F)
        strand(cds.frag.left) = strand(cds.span)[cds.frag.left$query.id]
        cds.frag.right = gr.findoverlaps(cds.span, gr.end(seg), qcol = c('gene_name', 'transcript_id'),  ignore.strand = F)
        strand(cds.frag.right) = strand(cds.span)[cds.frag.right$query.id]

        ## I want to find all unique walks that involve cds fragments
        ...
        start(cds.start.frag.pos) = start(cds.span)[cds.start.frag.pos$query.id]
        ...
        end(cds.end.frag.pos) = end(cds.span)[cds.end.frag.pos$query.id]
        ...
        end(cds.start.frag.neg) = end(cds.span)[cds.start.frag.neg$query.id]
        ...
        start(cds.end.frag.neg) = start(cds.span)[cds.end.frag.neg$query.id]
        ...
        edges = edges[cds.span$gene_name[all.frags$query.id[edges$i]] != cds.span$gene_name[all.frags$query.id[edges$j]] |
        ...
        paths = do.call('c', mclapply(1:length(vL), function(i) {
            ...
            if (length(x)==2)
                ...
                lapply(all.paths(A.frag[x,x, drop = FALSE], source.vertices = tmp.source, sink.vertices = tmp.sink, verbose = FALSE)$paths, function(y) x[y])
        }, mc.cores = 1))
        ...
        values(walks)$ab.id = sapply(values(walks)$seg.id, function(x)
            if (length(x)==1) c() else setdiff(A.ab[cbind(x[-length(x)], x[-1])], NA))

        walks = walks[sapply(values(walks)$ab.id, length)>0]      
...
# basis vectors v such that nz(exclude.ranges[i, ]) \subset nz(abs(A)*v)) for some pattern i.  Again
...
#
# output is a list of integer vectors corresponding to indices in A (ie vertices)
# $paths = paths indices
# $cycles = cycle indices
#
# if all = T, will include all sources (parentless vertices) and sinks (childless vertices) in path computation
# if ALL = T, will also include vertices without outgoing and incoming edges in paths
#####################################################
...
#################################################
# Given karyograph kag (output of karyograph)
# and ramip solution sol, outputs all chromoplexy paths and cycles
...
#
# returns paths and cycles of as list of vectors of aberrant edge index sequences,
# aberrant edges refer to edges described in kag$ab.edges matrix
#
# cn.dist is the minimum distance at which to enforce the strict copy change criteria (ie copy loss for deletoin
# bridges and copy gain for amp bridges)
#
# if paths = T, will also try to compute paths (in addition to cycles)
#################################################
...
#        G = graph.adjacency(adj2)
    ...
    ab.edges = cbind(rbind(kag$ab.edges[nnab, c('from', 'to'), '+'], kag$ab.edges[nnab, c('from', 'to'), '-']), junc.id = 1:nrow(kag$ab.edges))
...
#
# The output is a list of n x m sparse distance matrices:
    ...
    if (!is.null(jab))
      {
        ...
        if (length(ix)>0)
          {
            ra1 = gr.flip(gr.end(jab$segstats[jab$ab.edges[,1,1]][ix], 1, ignore.strand = F))
            ra2 = gr.start(jab$segstats[jab$ab.edges[,2,1]][ix], 1, ignore.strand = F)
    ...
    tmp = mclapply(ix.query, function(i)
      {
        ...
        D.which[i, ix] = ifelse(tmp.D1<tmp.D2, 1, 2)[ix]
...
#          cat('adding', i, '\n')
    ...
    if (!is.null(query.nm))
      sum$query.nm = query.nm

    if (!is.null(names(subject)))
      sum$subject.nm = names(subject)
    ...
    ## reconstruct paths
    vix.query = matrix(NA, nrow = length(qix.filt), ncol = 2, dimnames = list(NULL, c('start', 'end')))
    vix.subject = matrix(NA, nrow = length(six.filt), ncol = 2, dimnames = list(NULL, c('start', 'end')))
    vix.query[qix.filt, ] = cbind(values(gr)[ix.query, c('node.start')], values(gr)[ix.query, c('node.start')])
    vix.subject[six.filt] = cbind(values(gr)[ix.subj, c('node.start')], values(gr)[ix.subj, c('node.start')])
    
    sum.paths = mapply(function(x, y)
      {
        if ((ra.which[x, y]) == 1)
          get.shortest.paths(kg$G, vix.query[x, 'end'], vix.subject[y, 'start'], weights = E(kg$G)$weight, mode = 'out')[[1]]
        else
          get.shortest.paths(kg$G, vix.subject[y, 'end'], vix.query[x, 'start'], weights = E(kg$G)$weight, mode = 'out')[[1]]
...
# ppgrid
#
# quick and dirty grid search for purity and ploidy modes
# 
####################################################################
ppgrid = function(
  ...
  )
{
  ...
  purity.guesses = seq(pmin(0, purity.min), pmax(1.00, purity.max), purity.step)
...
#          tmp.out$col = brewer.master(length(levels(tmp.out$group)), 'PuRd')[as.integer(tmp.out$group)]
          ...
          legend(0, par('usr')[4]*0.98, legend = paste(tmp.out$rank, ') ', sapply(tmp.out$purity, function(x) sprintf('%0.2f',x)), ', ',
                                            sapply(tmp.out$ploidy, function(x) sprintf('%0.2f',x)), sep = ''), fill = tmp.out$col, cex = 0.8, yjust = 1, ncol = 1)         
      }
...
############################
rel2abs = function(gr, purity = NA, ploidy = NA, gamma = NA, beta = NA, field = 'signal', field.ncn = 'ncn')
  {        
    mu = values(gr)[, field]
...
## compute tumor / normal coverage
cov_stub = function(tum, norm, outfile, tiles.fn, mc.cores = 4, chunksize = 1e2)
  {
    require(Rsamtools);
    tiles = readRDS(tiles.fn)
    cat('tumnom_cov_stub\n')
    cat('Processing tumor \n')
    tum.bam = BamFile(tum)
    norm.bam = BamFile(norm)
    tum.counts = bam.cov.gr(tum.bam, tiles, mc.cores = mc.cores, verbose = T, chunksize = chunksize)
    cat('Finished tumor, doing normal \n');
    norm.counts = bam.cov.gr(norm.bam, tiles, mc.cores = mc.cores, verbose = T, chunksize = chunksize)
    
    if (any(na.ix <-is.na(tum.counts$records)))
      {
        cat('Reprocessing NA tumor ranges\n')
        tum.counts[na.ix] = bam.cov.gr(tum.bam, tiles[na.ix], mc.cores = mc.cores, verbose = T, chunksize = chunksize)
      }
    
    if (any(na.ix <-is.na(norm.counts$records)))
      {
        cat('Reprocessing NA normal ranges\n')
        norm.counts[na.ix] = bam.cov.gr(tum.bam, tiles[na.ix], mc.cores = mc.cores, verbose = T, chunksize = chunksize)
      }
    out = tiles[, c()]
    out$tum.counts = as.numeric(tum.counts$records)
    out$norm.counts = as.numeric(norm.counts$records)
    out$ratio = (out$tum.counts / out$norm.counts)*sum(as.numeric(out$norm.counts), na.rm = T)/sum(as.numeric(out$tum.counts), na.rm = T)

    ## gr.backtile is used to compute an attenuated normalization that is still reflects broad copy changes (eg sex chroms, trisomies)
    ## while correcting for local deviation of normal read counts from their broad / chromosome level context

    # use 5 MB tiles
    tmp.backtile = gr.tile(seqlengths(out), 5e6)
    smp.out = sample(out, pmin(1e5, length(out)))
    map = gr.tile.map(tmp.backtile, smp.out, verbose = T)
    val = smp.out$norm.counts
    val[val==0] = NA           
    tmp.backtile$mean = sapply(map, function(x,y) if (length(x)==0) NA else if ((sum(is.na(y[x]))/length(x))<.10) mean(y[x], na.rm = T) else NA, val)
    tmp.backtile = tmp.backtile[!is.na(tmp.backtile$mean)]

    # segment
    seg = segment(CNA(log(as.numeric(tmp.backtile$mean)), as.character(seqnames(tmp.backtile)), (end(tmp.backtile) + start(tmp.backtile))/2, data.type = 'logratio'))
    tmp.df = print(seg) ## rewire janky CNA output to include segment ends
    gr.backtile = sort(seg2gr(tmp.df, seqlengths = seqlengths(tmp.backtile)))
    gr.backtile$mean = exp(gr.backtile$seg.mean)
    gaps = setdiff(gr.stripstrand(seqinfo2gr(gr.backtile)), gr.backtile) ## fix gaps in CN object
    gaps$mean = tmp.backtile$mean[nearest(gaps, tmp.backtile)] ## fill in segments with nearest member on chromosome with the nearest value    
    gr.backtile = sort(c(gr.backtile[, 'mean'], gaps[, 'mean'])) ## combine    
    td.back = trackData(gr.backtile, y.field = 'mean', col = 'black', y0 = 0, y1 = 1600)

    map = gr.tile.map(gr.backtile, out, verbose = T)
    ix = rep(NA, length(out))
    ix[unlist(map)] = rep(1:length(map), sapply(map, length))
    normFactor = out$norm.counts / (1+gr.backtile$mean[ix]);
    out$ratio = out$tum.counts * sum(as.numeric(normFactor), na.rm = T ) / normFactor / sum(as.numeric(out$tum.counts), na.rm = T);
    out$ratio[unlist(map[which(is.na(gr.backtile$mean))])] = 0 ## any NA's in backtile come from "missing" chromosomes in normal
    
    td.out = trackData(sample(out, 1e5), y.field = 'ratio', y1 = 10)
#    ppng(display(c(td.ncounts, td.tcounts, td.cov, td.out, td.back), border = alpha('black', 0.5)))
...
karyograph_stub = function(seg.file, ## path to rds file of initial genome partition (values on segments are ignored)
  ...
  field = 'ratio', mc.cores = 1, max.chunk = 1e8, subsample = NULL)
  {
      ...
      else if (grepl('(\\.vcf$)|(\\.vcf\\.gz$)', ra.file))
          {        
...
#      if (is.character(tryCatch(png(paste(out.file, '.ppgrid.png', sep = ''), height = 500, width = 500), error = function(e) 'bla')))
      ...
      display(c(trackData(gr.fix(sample(this.cov, 5e4), this.kag$segstats), y.field = 'ratio', col = alpha('black', 0.3)),
                trackData(this.kag$segstats, y.field = 'mean', angle = 0, labels.suppress = T, col = 'gray10', border = alpha('black', 0.2))), links = this.kag$junctions, y1 = y1)
      dev.off()
...
ramip_stub = function(kag.file, out.file, mc.cores = 1, mem = 16, tilim = 1200, slack.prior = 0.001, gamma = NA, beta = NA, customparams = T,
  ...
  )
  {
    ...
    ra.sol = raMIP(this.kag$adj, this.kag$segstats, beta.guess = this.kag$beta, gamma.guess = this.kag$gamma, tilim = tilim, slack.prior = slack.prior, cn.prior = NA, mipemphasis = 0, ignore.cons = T, adj.nudge = adj.nudge, cn.ub = rep(500, length(this.kag$segstats)), verbose = T)
...
#
.pretty.jab  = function(this.jab)
  {
    ed = edgs(this.jab$td)[[1]]
    ed$v.override = NA
    ed$col[ed$type=='reference'] = "#BEBEBE33"
    ed[ed$type == 'loose', ]$h = ed[ed$type == 'loose', ]$h*2
                                        #ed[ed$type == 'aberrant', ]$v.override = ed[ed$type == 'aberrant', ]$v*0.02
    ed[ed$type == 'aberrant', ]$h = ed[ed$type == 'aberrant', ]$h*2
    ed[ed$type == 'aberrant', ]$lwd = ed[ed$type == 'aberrant', ]$lwd*0.5
    ed[ed$type == 'loose', ]$lwd = ed[ed$type == 'loose', ]$lwd*1
    ed[ed$type == 'loose', ]$lty = 1
    ed$dangle.w = 0.5
    segs = dat(this.jab$td)[[1]]
                                        #segs$ywid = segs$ywid*0.5
    td2 = trackData(segs, edges = ed)
    formatting(td2) = formatting(this.jab$td)
    this.jab$td = td2
    return(this.jab)
  }
