...
)
{
    ...
    if (overwrite | !file.exists(jabba.raw.rds.file))
      {
        ramip_stub(kag.file, jabba.raw.rds.file, mc.cores = cores, tilim = tilim, edge.nudge = edgenudge, ab.force = ab.force, mem = mem, init = init, 
...
### implement lower bounds and fixes
  ...
  #' mimielinski Sunday, Dec 31, 2017 03:58:45 PM
  ## implement FIXES ...!! smh
  if (any(!is.na(cn.fix)))
    ub[v.ix[!is.na(cn.fix)]] = lb[v.ix[!is.na(cn.fix)]] = cn.fix[!is.na(cn.fix)]
